...
import { type InvoiceBody, InvoiceBodySchema, type UpdateInvoiceBody, convertToDBType } from '@/types';
...
export const invoiceRoutes = new Elysia()
  ...
  .get('/invoices/:id', async ({ params: { id } }) => {
    try {
      const invoice = await db.select().from(invoices).where(eq(invoices.id, Number.parseInt(id))).limit(1);
  ...
  .put('/invoices/:id', async ({ params: { id }, body }: { params: { id: string }, body: UpdateInvoiceBody }) => {
    try {
      ...
      const updatedInvoice = await db.update(invoices).set(updateData).where(eq(invoices.id, Number.parseInt(id))).returning();
      if (!updatedInvoice.length) return { success: false, error: 'Invoice not found' };
      return { success: true, data: updatedInvoice[0] };
    } catch (error) {
      return { success: false, error: error.message };
    }
  }, {
    params: t.Object({ id: t.String({ description: 'Invoice ID', example: '1' }) }),
  ...
  .delete('/invoices/:id', async ({ params: { id } }) => {
    try {
      const deletedInvoice = await db.delete(invoices).where(eq(invoices.id, Number.parseInt(id))).returning();
      if (!deletedInvoice.length) return { success: false, error: 'Invoice not found' };
